...
function App() {
  ...
  const submitHandler = async (e) => {
    if(e.key === 'Enter') {
      ...
      // fetch(`api.openweathermap.org/data/2.5/weather?q=${city}&appid=${API_KEY}&units=metric`).then(res => res.json()).then(data => console.log(data))
      // fetch('api.openweathermap.org/data/2.5/weather?q=London&appid=5526440f87f0eae7d6b98fd71afce7cf&units=metric').then(res => res.json()).then(data => console.log(data))
      // fetch function here
      const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=London&appid=5526440f87f0eae7d6b98fd71afce7cf&units=metric`)
      if(!response.ok) {
        return
      }
